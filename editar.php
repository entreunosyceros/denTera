...
$stmtTrat = $pdo->query("SELECT * FROM tratamientos ORDER BY nombre ASC");
$tratamientos = $stmtTrat->fetchAll();

$stmtDoc = $pdo->query("SELECT * FROM doctores ORDER BY nombre ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($datos['doctor_id'] !== null && $datos['fecha'] !== '' && $datos['hora'] !== '') {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM citas WHERE doctor_id = ? AND fecha = ? AND hora = ? AND estado IN ('pendiente','confirmada') AND id != ?");
        $stmt->execute([$datos['doctor_id'], $datos['fecha'], $datos['hora'], $id]);
    ...
    if (empty($errores)) {
        ...
        $stmt = $pdo->prepare("UPDATE citas SET paciente=?, paciente_id=?, dni=?, telefono=?, email=?, fecha=?, hora=?, doctor_id=?, motivo=?, tratamiento_id=?, estado=?, forma_pago=?, notas=? WHERE id=?");
        $stmt->execute([
            $datos['paciente'],
            $paciente_id,
            $datos['dni'],
        ...
        header('Location: index.php?msg=actualizada');
        registrarAccion($pdo, 'editar', 'citas', $id, "Cita actualizada de {$datos['paciente']}");
        exit;
    }
}
?>
...
<head>
    ...
    <title>DenTera - Editar Cita #<?= $id ?></title>
    ...
    <div class="form-page-wide">
        ...
        <form method="POST" novalidate>
            ...
            <div class="form-actions-bar">
                ...
                <button type="submit" class="btn btn-primary btn-lg">Actualizar Cita</button>
            </div>
        </form>
    </div>
</main>
